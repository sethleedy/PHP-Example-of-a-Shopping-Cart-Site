	...
	if (isset($_GET['sw_id'])) { // Check for a product ID.
		...
		if ($sw_id > 0) {
			...
			try {
				...
				$statement = $link->prepare("SELECT name, color, size FROM general_widgets LEFT JOIN specific_widgets USING (gw_id) LEFT JOIN colors USING (color_id) LEFT JOIN sizes USING (size_id) WHERE sw_id=$sw_id");
				...
				//if ($statement->rowCount() == 1) {
					list($name, $color, $size) = $statement->fetch(PDO::FETCH_NUM);
				//}
